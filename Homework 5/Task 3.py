...
def everything_for_your_cat(cats_data):
    """
    Функция everything_for_your_cat преобразует список кортежей
    формата Кличка котика, Возраст котика, Имя покупателя и Фамилия покупателя
    в строку Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n

    :param cats_data: Книга о котах и покупателях
    :return: (our_str) Строка нового формата
    """
    owners = {}
    for cat_data in cats_data:
        name, age, owner_name, owner_surname = cat_data
        owner = (owner_name, owner_surname)
        if owner in owners:
            owners[owner].append((name, age))
        else:
            owners[owner] = [(name, age)]

    our_str = str()
    for owner, cats in owners.items():
        cat_str = '; '.join([f'{cat[0]}, {cat[1]}' for cat in cats])
        our_str += f'{owner[0]} {owner[1]}: {cat_str}\n'
    return our_str
