...
class PersonInfo:
    def __init__(self, name: str, age: int, *depts):
        """
        Класс Информация о сотруднике
        :param name: Имя Фамилия
        :param age: Возраст сотрудника
        :param depts: Подразделения от головного до того, где работает сотрудник.
        """
        self.name = name
        self.age = age
        self.subdivision = depts

    def short_name(self):
        """
        Метод возвращает короткое имя
        :return: Фамилия И.
        """
        s_name = self.name.split(' ')
        return f'{s_name[1]} {s_name[0][0]}.'

    def path_deps(self):
        """
        Возвращает путь до подразделения
    ...
    def new_salary(self):
        """
        Рассчитывает новую зарплату по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
