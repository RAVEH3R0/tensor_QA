...
class Bus(PublicTransport):
    def __init__(self, brand: str, engine_power: int, year: int, color: str, max_speed: int,
                 passengers: int, park: int, fare: int):
        """
        Класс автобус
        :param brand: Марка транспорта
        :param engine_power: Мощность двигателя
    ...
    def park(self, park):
        """
        Проверяет при присвоении номер парка, что он в диапазоне от 1000 до 9999
        :param park:
        :return:
        """
...
class Tram(PublicTransport):
    ...
    def how_long(self):
        """
        Вычисляет время прохождение маршрута по формуле max_speed/(4*path)
