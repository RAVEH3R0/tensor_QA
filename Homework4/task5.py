...
def josephus_task(num_people, kill_num):
    """
    Функция находит последний эл-т списка после удаления каждого kill_num по счёту
    :param num_people: Кол-во эл-тов в списке
    :param kill_num: Сколько эл-тов удаляется из списка
    :return: (survivor)
    """
    people = list(range(1, num_people + 1))
    kill_stat = 0
    while len(people) > 1:
        kill_stat += kill_num - 1
        if kill_stat > len(people) - 1:
            while kill_stat > len(people) - 1:
                kill_stat -= len(people)
        del people[kill_stat]
    survivor = people[0]
    return survivor
