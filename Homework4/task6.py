...
def create_phone_number(num_tuple):
    """
    Функция принимает кортеж num_tuple из 10 цифр num_tuple
    и возвращает строку этих чисел в виде номера телефона str_phone
    :param num_tuple: Кортеж цифр
    :return: (str_phone) Номер телефона согласно установленной маске
    """
    str_phone = "("
    for ind in range(len(num_tuple)):
        str_phone += str(num_tuple[ind])
        if ind == 2:
            str_phone += ") "
        if ind == 5:
            str_phone += "-"
